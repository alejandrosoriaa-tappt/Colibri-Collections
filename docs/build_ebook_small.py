...
OUT_HTML = os.path.join(HERE, 'ebook-mobile.html')
WEB = os.environ.get('EBOOK_WEB') == '1'
IMGDIR = 'ebook-assets/images-web-small' if WEB else 'ebook-assets/images'
...
img.cover { display:block; width:100%; border-radius:6px; margin:0 0 20px; }
...
.cover-page { text-align:left; page-break-after:always; }
...
.byline { font-size:12px; color:#5f5a50; margin-top:14px; }
...
def main():
    ...
    if title_line: cover_html.append(f'<h1>{inline(title_line.group(1), hl=False)}</h1>')
    if sub_line: cover_html.append(f'<p style="font-size:17px;color:#128C7E;font-family:Fraunces,Georgia,serif;">{inline(sub_line.group(1), hl=False)}</p>')
